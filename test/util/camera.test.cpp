...
TEST(FreeCameraOptions, SetLocation) {
    ...
    options.setLocation(LatLng{0.0, 0.0}, util::EARTH_RADIUS_M * M_PI);
    ASSERT_TRUE(options.position);
    ASSERT_THAT(options.position.value(), Vec3NearEquals(vec3{0.5, 0.5, 0.5}));

    options.setLocation(LatLng{25.0, -180.0}, 1000.0);
    ASSERT_TRUE(options.position);
    ASSERT_THAT(options.position.value(), Vec3NearEquals(vec3{0.0, 0.4282409625, 0.000027532812465}));

    options.setLocation(LatLng{util::LATITUDE_MAX, 0.0},
                        util::EARTH_RADIUS_M * M_PI * std::cos(util::LATITUDE_MAX * util::DEG2RAD));
    ASSERT_TRUE(options.position);
...
TEST(FreeCameraOptions, SetLocationNegativeAltitude) {
    FreeCameraOptions options;
    options.setLocation(LatLng{0.0, 0.0}, -100.0);
    ASSERT_TRUE(options.position);
...
TEST(FreeCameraOptions, SetLocationUnwrappedLocation) {
    ...
    options.setLocation(LatLng{0.0, -540.0}, 0.0);
    ASSERT_TRUE(options.position);
...
TEST(FreeCameraOptions, GetLocation) {
    ...
    std::tie(location, altitude) = options.getLocation().value();
    ASSERT_DOUBLE_EQ(location.latitude(), 0.0);
    ASSERT_DOUBLE_EQ(location.longitude(), 0.0);
    ASSERT_DOUBLE_EQ(altitude, 20037508.342789244);

    options.position = vec3{0.0, 0.4282409625, 0.000027532812465};
    ASSERT_TRUE(options.getLocation());
    std::tie(location, altitude) = options.getLocation().value();
    ASSERT_NEAR(location.latitude(), 25.0, 1e-7);
    ASSERT_NEAR(location.longitude(), -180.0, 1e-7);
    ASSERT_NEAR(altitude, 1000.0, 1e-7);

    options.position = vec3{0.5, 0.0, 0.5};
    ASSERT_TRUE(options.getLocation());
    std::tie(location, altitude) = options.getLocation().value();
    ASSERT_NEAR(location.latitude(), util::LATITUDE_MAX, 1e-7);
    ASSERT_NEAR(location.longitude(), 0.0, 1e-7);
    ASSERT_NEAR(altitude, 1728570.489074, 1e-6);
}
...
TEST(FreeCameraOptions, GetLocationNegativeAltitude) {
    ...
    const auto altitude = std::get<double>(latLngAltitude.value());
    ASSERT_DOUBLE_EQ(altitude, -0.5 * util::EARTH_RADIUS_M * M_PI);
}
...
TEST(FreeCameraOptions, GetLocationUnwrappedPosition) {
    ...
    LatLng latLng;
    double altitude;
    std::tie(latLng, altitude) = latLngAltitude.value();
    ASSERT_DOUBLE_EQ(latLng.longitude(), 270.0);
    ASSERT_DOUBLE_EQ(latLng.latitude(), 0.0);
    ASSERT_DOUBLE_EQ(altitude, 0.0);
}
