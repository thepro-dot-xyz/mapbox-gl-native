...
static double vec2Len(const vec2& v) {
    return std::sqrt(v[0] * v[0] + v[1] * v[1]);
};

static double vec2Dot(const vec2& a, const vec2& b) {
    return a[0] * b[0] + a[1] * b[1];
};

static vec2 vec2Scale(const vec2& v, double s) {
...
void FreeCameraOptions::setLocation(const LatLng& location, double altitudeMeters) {
    position = util::toMercator(location, altitudeMeters);
}

optional<std::tuple<LatLng, double>> FreeCameraOptions::getLocation() {
    if (!position) {
        return nullopt;
    }
...
void FreeCameraOptions::lookAtPoint(const LatLng& location, optional<vec3> upVector) {
    orientation = nullopt;
    if (!position) {
        return;
    }
